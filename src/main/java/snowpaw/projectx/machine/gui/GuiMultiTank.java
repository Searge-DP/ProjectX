...
public class GuiMultiTank extends GuiScreen {
	
    protected static final ResourceLocation tex = new ResourceLocation(ProjectX.INSTANCE, "textures/gui/gui_tank.png");
    protected static final int AUTO_FLUID_OUTPUT_BTN_ID = 23442;

    TileXTankValve valve;
    boolean isFrame;
    ...
    public GuiMultiTank(TileXTankValve valve, boolean isFrame) {
        super();
    ...
    public void initGui() {
        ...
        if(!isFrame) {
            this.buttonList.add(new ButtonToggle(AUTO_FLUID_OUTPUT_BTN_ID, this.left + 80, this.top + 20, "Auto fluid output", this.valve.getAutoOutput(), 16777215));
    ...
    public void onGuiClosed() {
        ...
        if(!isFrame)
            if(!valveName.getText().isEmpty())
                NetworkHandler.sendPacketToServer(new FluidPacket.Server.UpdateValveName(valve, valveName.getText()));
    }
    ...
    private void fluidHoveringText(String fluid) {
        if(mouseX >= left + 10 && mouseX < left + 10 + 64 &&
            ...
            texts.add(fluid);
            texts.add("\u00A7" + EnumChatFormatting.GRAY.getFormattingCode() + (FluidUtils.intToFancyNumber(this.valve.getFluidAmount()) + " / " + FluidUtils.intToFancyNumber(this.valve.getCapacity())) + " mB");
    ...
    public void actionPerformed(GuiButton btn) {
        if (btn.id == AUTO_FLUID_OUTPUT_BTN_ID && btn instanceof ButtonToggle) {
            ButtonToggle toggle = (ButtonToggle)btn;

            this.valve.setAutoOutput(toggle.getState());
            NetworkHandler.sendPacketToServer(new FluidPacket.Server.UpdateAutoOutput(this.valve, this.valve.getAutoOutput()));
        }
    }

    private void drawFluid(int x, int y) {
        IIcon fluidIcon = FluidRenderHelper.getFluidTexture(valve.getFluid().getFluid(), false);
        if(fluidIcon == null)
            return;

        this.mc.getTextureManager().bindTexture(FluidRenderHelper.BLOCK_TEXTURE);
